...
# ==================================================================================================
# Tests for Client Thinking Effort Support (Issue #111, native effort migration)
# ==================================================================================================
...
class TestExtractThinkingConfigFromOpenAI:
    ...
    def test_no_reasoning_effort(self):
        """
        What it does: Verifies ThinkingConfig(enabled=True, effort=None) when reasoning_effort=None
    ...
    def test_reasoning_effort_minimal_aliased_to_low(self):
        """
        What it does: Verifies reasoning_effort="minimal" maps to the canonical "low" tier
        Purpose: Ensure OpenAI-specific aliases resolve to schema-safe tiers
        """
        print("Creating request with reasoning_effort='minimal'...")
        request = ChatCompletionRequest(
            model="claude-sonnet-4.5",
            messages=[ChatMessage(role="user", content="test")],
    ...
    def test_reasoning_effort_high_preserved_verbatim(self):
        """
        What it does: Verifies reasoning_effort="high" is preserved as a qualitative tier
        Purpose: Ensure effort is never converted into a numeric budget
        """
    ...
    def test_reasoning_effort_case_normalized(self):
        """
        What it does: Verifies reasoning_effort="High" normalizes to "high"
        Purpose: Ensure casing and whitespace do not leak into the native field
        """
        print("Creating request with reasoning_effort=' High '...")
        request = ChatCompletionRequest(
            model="claude-sonnet-4.5",
            messages=[ChatMessage(role="user", content="test")],
            reasoning_effort=" High "
        ...
        print(f"Comparing: expected='high', got='{config.effort}'")
        assert config.effort == "high"

    def test_reasoning_effort_unknown_uses_fallback(self):
        """
        What it does: Verifies an unknown reasoning_effort falls back to the default tier
        Purpose: Ensure defensive default instead of passing garbage to Kiro
        """
        print("Creating request with reasoning_effort='ultra'...")
        request = ChatCompletionRequest(
            model="claude-sonnet-4.5",
            messages=[ChatMessage(role="user", content="test")],
        ...
        print(f"Comparing: expected='medium', got='{config.effort}'")
        assert config.enabled is True
...
class TestBuildKiroPayloadIntegration:
    ...
    def test_supported_model_sends_native_effort(self, monkeypatch):
        ...
        print("Creating request for claude-opus-5 with reasoning_effort='high'...")
        request = ChatCompletionRequest(
            model="claude-opus-5",
    ...
    def test_unsupported_model_falls_back_to_effort_tag(self, monkeypatch):
        ...
        print("Creating request for claude-sonnet-4.5 with reasoning_effort='medium'...")
        request = ChatCompletionRequest(
            model="claude-sonnet-4.5",
            messages=[ChatMessage(role="user", content="Test message")],
        ...
        print("Checking no native fragment was sent...")
        assert "additionalModelRequestFields" not in payload

        print("Checking <thinking_effort>medium</thinking_effort> tag injection...")
        content = payload["conversationState"]["currentMessage"]["userInputMessage"]["content"]
        assert "<thinking_mode>enabled</thinking_mode>" in content
        assert "<thinking_effort>medium</thinking_effort>" in content
        # The system-prompt documentation mentions <max_thinking_length>; only a
    ...
    def test_gpt_model_receives_native_none_when_disabled(self, monkeypatch):
        ...
        print("Creating request for gpt-5.5 with reasoning_effort='none'...")
        request = ChatCompletionRequest(
            model="gpt-5.5",
            messages=[ChatMessage(role="user", content="Test message")],
            reasoning_effort="none"
        ...
        print("Checking native none fragment...")
        assert payload["additionalModelRequestFields"] == {
