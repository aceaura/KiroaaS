...
def get_model_id_for_kiro(
    ...
    Aliases are resolved first, mirroring ModelResolver.resolve()'s Layer 0. The
    converters are the only place the request model reaches the payload -- the routes
    never call ModelResolver.resolve() -- so without this the alias table would be
    advertised by /v1/models yet rejected by Kiro (e.g. "auto-kiro" → 400 while its
    target "auto" works).

    Args:
        model_name: External model name from client
        hidden_models: Dict mapping display names to internal Kiro IDs
        aliases: Dict mapping alias names to real model IDs (MODEL_ALIASES)
    ...
    Examples:
        ...
        >>> get_model_id_for_kiro("claude-3-7-sonnet", {"claude-3.7-sonnet": "CLAUDE_3_7_SONNET_20250219_V1_0"})
        'CLAUDE_3_7_SONNET_20250219_V1_0'
        >>> get_model_id_for_kiro("auto-kiro", {}, {"auto-kiro": "auto"})
        'auto'
    """
    # Layer 0: alias → real model ID. Matched on the raw name, exactly as
    # ModelResolver.resolve() does, so alias keys may use any spelling.
    if aliases:
        resolved = aliases.get(model_name, model_name)
        if resolved != model_name:
            logger.debug(f"Alias resolved: '{model_name}' -> '{resolved}'")
            model_name = resolved
